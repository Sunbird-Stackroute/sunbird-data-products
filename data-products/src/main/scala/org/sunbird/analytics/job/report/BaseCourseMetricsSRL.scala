...
case class BaseCourseMetricsOutputSRL(courseName: String, batchName: String, status: String, slug: String, courseId: String, batchId: String) extends AlgoInput

trait BaseCourseMetricsSRL[T <: AnyRef, A <: BaseCourseMetricsOutputSRL, B <: AlgoOutput, R <: Output] extends IBatchModelTemplate[T,BaseCourseMetricsOutputSRL,B,R]{

  override def preProcess(events: RDD[T], config: Map[String, AnyRef])(implicit sc: SparkContext, fc: FrameworkContext): RDD[BaseCourseMetricsOutputSRL] = {
    ...
    val encoder = Encoders.product[BaseCourseMetricsOutputSRL]
    data.as[BaseCourseMetricsOutputSRL](encoder).rdd
  }
